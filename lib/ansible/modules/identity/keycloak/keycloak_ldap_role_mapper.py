    ...
    def __init__(self, module, connection_header):
        ...
        if self.module.params.get('mapper_name'):
            ...
            if not self.federation.uuid:
                raise KeycloakError(
                    'Cannot access mapper because {given_id} federation does not exist.'.format(
                        given_id=self.federation.given_id
                    )
                )
        else:
        ...
        try:
            ...
            if self.initial_representation['providerId'] != 'role-ldap-mapper':
                raise KeycloakError(
                    '{given_id} is not a role mapper.'.format(given_id=self.given_id)
                )
...
def run_module():
    meta_args = dict(
        ...
        memberof_ldap_attribute=dict(
            type=str
        ),  # used only if user roles retrieve strategy is GET_ROLES_FROM_USER_MEMBEROF_ATTRIBUTE
        use_realm_roles_mapping=dict(type=bool),
        # the value should be in the list of client id of the realm (to check)
        client_id=dict(type=str),  # used only if user realm roles mapping is False.
    )

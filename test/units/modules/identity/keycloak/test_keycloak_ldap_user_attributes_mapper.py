...
from ansible.modules.identity.keycloak import keycloak_ldap_user_attributes_mapper
...
def test_mapper_name_without_federation_id_should_fail(monkeypatch):
    monkeypatch.setattr(keycloak_ldap_user_attributes_mapper.AnsibleModule, 'exit_json', exit_json)
    monkeypatch.setattr(keycloak_ldap_user_attributes_mapper.AnsibleModule, 'fail_json', fail_json)
    arguments = {
    ...
    with pytest.raises(AnsibleFailJson) as exec_error:
        keycloak_ldap_user_attributes_mapper.run_module()
    ...
    assert not ansible_exit_json['user_attribute_mapper']
    ...
    existing_federation.update(
        {
            ...
            'http://keycloak.url/auth/admin/realms/master/components?parent=456-456&type=org.keycloak.storage.ldap.mappers.LDAPStorageMapper&name=user-attributes-mapper1': create_wrapper(
...
def test_good_name_but_wrong_type_should_raise_an_error(monkeypatch, mock_wrong_type):
    ...
    arguments = {
        ...
        'mapper_name': 'user-attributes-mapper1',
    }
    set_module_args(arguments)
    ...
    with pytest.raises(AnsibleFailJson) as exec_error:
        keycloak_ldap_user_attributes_mapper.run_module()
    ansible_exit_json = exec_error.value.args[0]
    assert ansible_exit_json['msg'] == 'user-attributes-mapper1 is not a user attributes mapper.'
    assert not ansible_exit_json['changed']
    assert not ansible_exit_json['user_attribute_mapper']
...
@pytest.mark.parametrize(
    'extra_arguments',
    [
        {'mapper_name': 'user-attributes-mapper1', 'federation_id': 'my-company-ldap'},
        {'mapper_uuid': '123-123'},
    ],
    ids=['mapper and federation names', 'mapper uuid'],
)
def test_present_user_attributes_mapper_without_properties_should_do_nothing(
    ...
    assert ansible_exit_json[
        'msg'
    ] == 'User attributes mapper {} up to date, doing nothing.'.format(given_id)
    assert not ansible_exit_json['changed']
    assert ansible_exit_json['user_attribute_mapper'] == {
        'id': '123-123',
        'name': 'user-attributes-mapper1',
        'providerId': 'user-attribute-ldap-mapper',
        'providerType': 'org.keycloak.storage.ldap.mappers.LDAPStorageMapper',
...
def test_state_absent_should_delete_existing_mapper(monkeypatch, mock_delete_url):
    ...
    with pytest.raises(AnsibleExitJson) as exec_error:
        keycloak_ldap_user_attributes_mapper.run_module()
    ansible_exit_json = exec_error.value.args[0]
    assert ansible_exit_json['msg'] == 'User attributes mapper user-attributes-mapper1 deleted.'
...
def mock_update_url(mocker):
    ...
    updated_mapper['config']['ldap.attribute'] = ['first_name']
    updated_mapper['config']['read.only'] = ['false']
    existing_federation.update(
        {
            'http://keycloak.url/auth/admin/realms/master/components?parent=master&type=org.keycloak.storage.UserStorageProvider&name=my-company-ldap': create_wrapper(
                json.dumps(
...
def test_state_present_should_update_existing_mapper(monkeypatch, mock_update_url):
    ...
    with pytest.raises(AnsibleExitJson) as exec_error:
        keycloak_ldap_user_attributes_mapper.run_module()
    ansible_exit_json = exec_error.value.args[0]
    assert ansible_exit_json['msg'] == 'User attributes mapper user-attributes-mapper1 updated.'
    assert ansible_exit_json['changed']
    ...
    assert ansible_exit_json['user_attribute_mapper'] == clean_payload_with_config(
        reference_arguments
    )


def test_incompatible_arguments_should_fail(monkeypatch, mock_update_url):
    monkeypatch.setattr(keycloak_ldap_user_attributes_mapper.AnsibleModule, 'exit_json', exit_json)
    monkeypatch.setattr(keycloak_ldap_user_attributes_mapper.AnsibleModule, 'fail_json', fail_json)
    arguments = {
        'auth_keycloak_url': 'http://keycloak.url/auth',
    ...
    existing_federation.update(
        {
            ...
            'http://keycloak.url/auth/admin/realms/master/components?parent=456-456&type=org.keycloak.storage.ldap.mappers.LDAPStorageMapper&name=user-attributes-mapper1': [
                create_wrapper(json.dumps({})),
                create_wrapper(json.dumps(MAPPER_DICT)),
            ],
...
def test_present_state_should_create_absent_mapper(monkeypatch, mock_create_url):
    ...
    reference_final_state = deepcopy(MAPPER_DICT)

    assert ansible_exit_json['msg'] == 'User attributes mapper user-attributes-mapper1 created.'
    assert ansible_exit_json['changed']
    assert ansible_exit_json['user_attribute_mapper'] == clean_payload_with_config(
        reference_final_state
    )
    create_call_arguments = json.loads(mock_create_url.mock_calls[3][2]['data'])
    reference_payload = deepcopy(reference_final_state)
    reference_payload.pop('id')
    assert create_call_arguments == reference_payload


def test_missing_argument_for_creation_should_raise_an_error(monkeypatch, mock_create_url):
    monkeypatch.setattr(keycloak_ldap_user_attributes_mapper.AnsibleModule, 'exit_json', exit_json)
    monkeypatch.setattr(keycloak_ldap_user_attributes_mapper.AnsibleModule, 'fail_json', fail_json)
    arguments = {
        'auth_keycloak_url': 'http://keycloak.url/auth',
    ...
    assert ansible_exit_json['msg'] == (
        'ldap_attribute and user_model_attribute are mandatory for creating a user attributes '
        'mapper.'
    )
    assert not ansible_exit_json['changed']
    assert not ansible_exit_json['user_attribute_mapper']
